...
class DeptDao:
    ...
    @classmethod
    def get_dept_info_for_edit_option(cls, db: Session, dept_info: DeptModel):
        """
        获取部门编辑对应的在用部门列表信息
        :param db: orm对象
    ...
    @classmethod
    def get_dept_list_for_tree(cls, db: Session, dept_info: DeptModel):
        """
        获取所有在用部门列表信息
        :param db: orm对象
        :param dept_info: 部门对象
            ...
            if dept_query_all:
                for dept_query in dept_query_all:
                    ...
                    for ancestor in ancestor_info:
                        dept_item = cls.get_dept_by_id(db, int(ancestor))
                        if dept_item:
    ...
    @classmethod
    def get_dept_list(cls, db: Session, page_object: DeptModel):
        """
        根据查询参数获取部门列表信息
        :param db: orm对象
        ...
        if page_object.dept_name or page_object.status:
            ...
            if dept_query_all:
                for dept_query in dept_query_all:
                    ...
                    ancestor_info.append(dept_query.dept_id)
                    for ancestor in ancestor_info:
                        dept_item = cls.get_dept_by_id_for_list(db, int(ancestor))
                        if dept_item:
                            dept.append(dept_item)
            # 去重
            dept_result = list(set(dept))
