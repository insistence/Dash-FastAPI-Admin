...
def get_menu_info_for_edit_option(db: Session, menu_info: MenuModel, user_id: int, role: list):
    menu_result = []
    for item in role:
        if item.role_id == 1:
...
def get_menu_list_for_tree(db: Session, menu_info: MenuModel, user_id: int, role: list):
    menu_query_all = []
    for item in role:
        if item.role_id == 1:
...
def get_menu_list(db: Session, page_object: MenuModel, user_id: int, role: list):
    """
    根据查询参数获取菜单列表信息
    :param db: orm对象
