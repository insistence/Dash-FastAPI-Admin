...
def config_confirm(
    ...
    if confirm_trigger:
        ...
        if all(
            ...
            if modal_type == 'add':
                api_res = ConfigApi.add_config(params_add)
            if modal_type == 'edit':
                api_res = ConfigApi.update_config(params_edit)
...
def config_delete_confirm(delete_confirm, config_ids_data):
    ...
    if delete_confirm:
        ...
        delete_button_info = ConfigApi.del_config(params)
...
def refresh_config_cache(refresh_click):
    ...
    """
    if refresh_click:
        refresh_info_res = ConfigApi.refresh_cache()
