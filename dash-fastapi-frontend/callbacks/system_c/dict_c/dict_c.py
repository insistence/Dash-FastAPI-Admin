...
def dict_type_confirm(
    ...
    if confirm_trigger:
        ...
        if all(
            ...
            if modal_type == 'add':
                api_res = DictTypeApi.add_type(params_add)
            if modal_type == 'edit':
                api_res = DictTypeApi.update_type(params_edit)
...
def dict_type_delete_confirm(delete_confirm, dict_ids_data):
    ...
    if delete_confirm:
        ...
        delete_button_info = DictTypeApi.del_type(params)
...
def refresh_dict_cache(refresh_click):
    ...
    """
    if refresh_click:
        refresh_info_res = DictTypeApi.refresh_cache()
