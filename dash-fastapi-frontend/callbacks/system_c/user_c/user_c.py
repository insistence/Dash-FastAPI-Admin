...
def add_user_modal(add_click):
    ...
    if add_click:
        ...
        if tree_info['code'] == 200 and detail_info['code'] == 200:
...
def usr_add_confirm(add_confirm, post, role, form_value, form_label):
    if add_confirm:
        ...
        if all(
            ...
            add_button_result = UserApi.add_user(params)
...
def usr_edit_confirm(
    ...
    if edit_confirm:
        ...
        if all(
            ...
            edit_button_result = UserApi.update_user(params)
...
def table_switch_user_status(
    ...
    if recently_switch_data_index:
        edit_button_result = UserApi.change_user_status(
...
def user_delete_confirm(delete_confirm, user_ids_data):
    ...
    if delete_confirm:
        ...
        delete_button_info = UserApi.del_user(params)
...
def user_reset_password_confirm(reset_confirm, user_id_data, reset_password):
    ...
    """
    if reset_confirm:
        reset_button_info = UserApi.reset_user_pwd(
            user_id=user_id_data, password=reset_password
        )
